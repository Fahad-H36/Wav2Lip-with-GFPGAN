...
set "image_path={{input_image_path}}"
set "input_video=input_videos"
set "input_audio={{input_audio_path}}"
...
python video2frames.py --input_video  %root%/results/result_voice.mp4 --frames_path  %root%/%frames_wav2lip%/%filename%
python GFPGAN\inference_gfpgan.py -i %frames_wav2lip%\%filename% -o  %root%/GFPGAN\results -v 1.3 -s 2
ffmpeg -framerate 24 -i  %root%/GFPGAN\results\restored_imgs\frame_%%05d.jpg -i  %input_audio% -c:v libx264 -c:a aac -strict experimental -b:a 192k -shortest results\%filename%_final_result.mp4
rmdir /s /q  %root%/%frames_wav2lip%\%filename%
rmdir /s /q  %root%/GFPGAN\results\*
